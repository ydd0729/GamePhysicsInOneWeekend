...
bool SwapChain::Create(DeviceContext* device, int width, int height)
{
    ...
    //
    {
        ...
        if (physicalDeviceInfo.m_vkSurfaceCapabilities.maxImageCount > 0 && imageCount > physicalDeviceInfo.
            m_vkSurfaceCapabilities.maxImageCount)
        {
            imageCount = physicalDeviceInfo.m_vkSurfaceCapabilities.maxImageCount;
        }

        uint32_t queueFamilyIndices[] = {
            static_cast<uint32_t>(device->m_graphicsFamilyIdx),
            static_cast<uint32_t>(device->m_presentFamilyIdx)
        };
    ...
    //
    {
        ...
        allocInfo.memoryTypeIndex = device->FindMemoryTypeIndex(memRequirements.memoryTypeBits,
...
uint32_t SwapChain::BeginFrame(DeviceContext* device)
{
    ...
    result = vkAcquireNextImageKHR(device->m_vkDevice, m_vkSwapChain, std::numeric_limits<uint64_t>::max(),
                                   m_vkImageAvailableSemaphore, VK_NULL_HANDLE, &m_currentImageIndex);
    ...
    vkResetCommandBuffer(device->m_vkCommandBuffers[m_currentImageIndex],
