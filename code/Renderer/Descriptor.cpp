...
bool Descriptors::Create(DeviceContext* device, const CreateParms_t& parms)
{
    ...
    //
    auto uniformBindings = static_cast<VkDescriptorSetLayoutBinding*>(alloca(
        sizeof(VkDescriptorSetLayoutBinding) * (numUniforms)));
...
void Descriptor::BindDescriptor(DeviceContext* device, VkCommandBuffer vkCommandBuffer, Pipeline* pso)
{
    ...
    auto descriptorWrites = static_cast<VkWriteDescriptorSet*>(alloca(allocationSize));
    ...
    vkUpdateDescriptorSets(device->m_vkDevice, static_cast<uint32_t>(numDescriptors), descriptorWrites, 0, nullptr);
    vkCmdBindDescriptorSets(vkCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pso->m_vkPipelineLayout, 0, 1,
                            &m_parent->m_vkDescriptorSets[m_id], 0, nullptr);
}
