...
{
    Vec3 pt;
    pt.x = static_cast<float>(data[0]) / 255.0f; // 0,1
    pt.y = static_cast<float>(data[1]) / 255.0f; // 0,1
    pt.z = static_cast<float>(data[2]) / 255.0f; // 0,1
...
{
    constexpr int numIdxs = 3 * 2 * 6;
    constexpr int numVerts = 4 * 6;
...
void FillCubeTessellated(Model& model, int numDivisions)
{
    ...
    auto v = static_cast<Vec3*>(malloc(numnum * sizeof(Vec3)));
    auto st = static_cast<Vec2*>(malloc(numnum * sizeof(Vec2)));
    for (int y = 0; y < numDivisions + 1; y++)
    {
        for (int x = 0; x < numDivisions + 1; x++)
        {
            float xf = ((static_cast<float>(x) / static_cast<float>(numDivisions)) - 0.5f) * 2.0f;
            float yf = ((static_cast<float>(y) / static_cast<float>(numDivisions)) - 0.5f) * 2.0f;
            v[y * (numDivisions + 1) + x] = Vec3(xf, yf, 1.0f);

            float sf = static_cast<float>(x) / static_cast<float>(numDivisions);
            float tf = static_cast<float>(y) / static_cast<float>(numDivisions);
    ...
    int faceIdx = 0;
    auto faceIdxs = static_cast<int*>(malloc(3 * 2 * numFaces * sizeof(int)));
    ...
    auto cubeVerts = static_cast<vert_t*>(malloc(numVerts * sizeof(vert_t)));
    auto cubeIdxs = static_cast<int*>(malloc(numIdxs * sizeof(int)));
...
void FillSphere(Model& model, const float radius)
{
    ...
    FillCubeTessellated(model, static_cast<int>(s));
...
bool Model::BuildFromShape(const Shape* shape)
{
    ...
    {
        auto shapeBox = static_cast<const ShapeBox*>(shape);
    ...
    {
        auto shapeSphere = static_cast<const ShapeSphere*>(shape);
    ...
    {
        auto shapeConvex = static_cast<const ShapeConvex*>(shape);
...
bool Model::MakeVBO(DeviceContext* device)
{
    ...
    bufferSize = static_cast<int>(sizeof(m_vertices[0]) * m_vertices.size());
    ...
    // Create Index Buffer
    bufferSize = static_cast<int>(sizeof(m_indices[0]) * m_indices.size());
