...
bool GetFileData(const char* fileNameLocal, unsigned char** data, unsigned int& size)
{
    ...
    *data = static_cast<unsigned char*>(malloc((size + 1) * sizeof(unsigned char)));
    ...
    // read the data
    unsigned int bytesRead = static_cast<unsigned int>(fread(*data, sizeof(unsigned char), size, file));
    fflush(file);
